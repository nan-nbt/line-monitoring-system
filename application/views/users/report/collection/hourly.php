...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed text-sm">
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header">
									<!-- form start -->
									<form class="form-horizontal" id="hourly-query">
										<div class="row">
											<div class="col-md-3">
												<!-- Date -->
												<div class="form-group">
													<label for="startdate"><span class="text-danger">*</span>Date :</label>
								...
								<!-- /.card-header -->
								<div class="card-body tab-content tab-pane fade show active">
									<table id="example2" class="table table-bordered table-striped table-head-fixed nowrap text-nowrap" style="width: 100%;">
										...
										<tbody id="data-collection">
											<!-- using JQuery to get data hourly defect report -->
										</tbody>
									</table>
								</div>
								<!-- /.card-body -->
	...
	<script>
		...
		$('#hourly-query').on('submit', function() {
		...
		function loadData() {
			...
			$.ajax({
				url: "<?php echo base_url('users/Collection/getHourly'); ?>",
				method: "POST",
				data: {
					area: area,
					build: build,
				...
				success: function(data) {
					...
					if (data.length > 0) {
						for (i = 0; i < data.length; i++) {
							var pass_rate = (data[i].pass_qty / data[i].check_qty * 100).toFixed(2);
							var defect_rate = (data[i].defect_qty / data[i].check_qty * 100).toFixed(2);

							if (pass_rate >= 90) {
								light = '<i class="fas fa-circle" style="color:#28a745;"></i>';
								cell = 'cell-green';
								bg_cell = 'bg-success';
							} else if (pass_rate < 90 && pass_rate >= 60) {
							...
							htmlData += '<tr>' +
								'<td class="' + cell + '">' + no++ + '</td>' +
								'<td>' + data[i].prod_fact + ' | ' + data[i].prod_fact_nm + '</td>' +
								'<td>' + data[i].build_no + ' | ' + data[i].build_eng_nm + '</td>' +
								'<td>' + data[i].sec_no + ' | ' + data[i].sec_name + '</td>' +
								'<td>' + data[i].time_1 + '</td>' +
								'<td>' + data[i].time_2 + '</td>' +
								'<td>' + data[i].time_3 + '</td>' +
								'<td>' + data[i].time_4 + '</td>' +
								...
								'<td>' + data[i].time_8 + '</td>' +
								'<td>' + data[i].time_9 + '</td>' +
								'<td>' + data[i].time_10 + '</td>' +
								'<td>' + data[i].time_11 + '</td>' +
								'<td>' + data[i].time_12 + '</td>' +
								'<td>' + data[i].time_13 + '</td>' +
								'<td>' + data[i].defect_qty + '</td>' +
								'<td>' + defect_rate + '</td>' +
								'<td>' + data[i].pass_qty + '</td>' +
								'<td class="' + bg_cell + '">' + pass_rate + '</td>' +
						...
						table = $('#example2').DataTable({
							...
							"buttons": [{
									...
									filename: 'Pouchen_<?php echo $this->session->userdata('sap_factory'); ?>_Traffic Light System Collection Hourly_<?php echo date('YmdHis'); ?>',
									messageTop: 'Data Collection Defect Hourly Report | Area: ' + area + ' | Building: ' + build + ' | Date: ' + startdate,
