...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed text-sm">
	...
	<script>
		...
		function loadData() {
			$.ajax({
				url: "<?php echo base_url('users/Defect/getList'); ?>",
				async: true,
				dataType: 'JSON',
					...
					if (data.length > 0) {
						for (i = 0; i < data.length; i++) {
							...
							if (data[i].line_type != null) {
								...
								var display = '<a href="javascript:void(0);" class="btn btn-sm p-0 setup" data-id="' + data[i].defect_no + '" data-nm="' + data[i].defect_name + '"><i class="fas fa-cog"></i> Setting</a>';
							}
